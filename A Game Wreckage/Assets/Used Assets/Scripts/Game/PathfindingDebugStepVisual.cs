...
public class PathfindingDebugStepVisual : MonoBehaviour {
    ...
    public void Setup(Grid<PathNode> grid) {
        ...
        for (int x = 0; x < grid.GetWidth(); x++) {
            for (int y = 0; y < grid.GetHeight(); y++) {
                Vector3 gridPosition = new Vector3(x, y) * grid.GetCellSize() + new Vector3(-9,-9) * grid.GetCellSize() * .5f;
        ...
        for (int x = 0; x < grid.GetWidth(); x++) {
            for (int y = 0; y < grid.GetHeight(); y++) {
                ...
                gridSnapshotAction.AddAction(() => {
                    ...
                    visualNode.Find("SquareOutline").GetComponent<SpriteRenderer>().color = backgroundColor;
                });
            }
        }
    ...
    private void SetupVisualNode(Transform visualNodeTransform, int gCost, int hCost, int fCost) {
        if (fCost < 1000) {
            visualNodeTransform.Find("gCostText").GetComponent<TextMeshProUGUI>().SetText(gCost.ToString());
            visualNodeTransform.Find("hCostText").GetComponent<TextMeshProUGUI>().SetText(hCost.ToString());
            visualNodeTransform.Find("fCostText").GetComponent<TextMeshProUGUI>().SetText(fCost.ToString());
        } else {
            visualNodeTransform.Find("gCostText").GetComponent<TextMeshProUGUI>().SetText("");
            visualNodeTransform.Find("hCostText").GetComponent<TextMeshProUGUI>().SetText("");
            visualNodeTransform.Find("fCostText").GetComponent<TextMeshProUGUI>().SetText("");
        }
    }
